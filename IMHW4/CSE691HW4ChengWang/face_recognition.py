...
if __name__ == '__main__':
    ...
    # ei2 = f.method_second()
    # f.method_one()
    f.find_n_eigenvector(50, eigenvalue, eigenvector)
    f.calssify(50)
    # f.show_eigenfaces()
    # f.show_img_average()
    ...
    # t7.reconstruct()
    # t7.image_diff()
    # t8 = TestImg(f.eigenfaces_, f.img_aver_, 'TestSet3/subject06.normal', f.face_class_)
    # t8.projection()
    # t8.reconstruct()
    # t8.image_diff()
    ...
    testpath = 'TestSet3'
    face = [join(testpath, f) for f in listdir(testpath) if isfile(join(testpath, f))]
    print(face)
    t = [TestImg(f.eigenfaces_, f.img_aver_, face[0], f.face_class_)] * len(face)
    for index, image in enumerate(face):
        t[index] = TestImg(f.eigenfaces_, f.img_aver_, image, f.face_class_)
        t[index].projection()
        t[index].reconstruct()
        # t[index].image_diff()
        t[index].find_class(10000)
    # plt.figure()
